...
# -*- coding: utf-8 -*-
"""
D-ProtoCoT / ORM 数据预处理脚本
功能：Parquet读取 → LLM路径生成 → 答案提取与清洗 → 格式转换 → Train/Val/Test划分
适配格式：{"question": "...", "ground_truth": "...", "paths": [{"reasoning": "...", "predicted_answer": "..."}]}
"""
...
def generate_k_paths(question, gen_pipe, tokenizer, k=10, temperature=0.7, top_p=0.9, max_new_tokens=1024):
    """为单个问题生成K条独立CoT路径"""
    paths = []
    # 使用 Chat Template 确保模型遵循指令
    messages = [{"role": "user", "content": f"Solve the following problem step by step. End your response with the final answer.\n\nQuestion: {question}"}]
    ...
    for i in range(k):
        try:
            outputs = gen_pipe(
                ...
                top_p=top_p,
                do_sample=True,
                return_full_text=False, # 仅返回新生成部分
            ...
            logger.warning(f"Failed generation #{i+1} for question: {e}")
...
def main():
    ...
    a_col = "ground_truth" if "ground_truth" in df.columns else ("answer" if "answer" in df.columns else df.columns[1])
    logger.info(f"Detected columns -> Question: '{q_col}', Answer: '{a_col}'")
    ...
    if os.path.exists(ckpt_path):
        ...
        logger.info(f"Resuming from checkpoint. Already processed: {start_idx}/{total_q}")
    else:
        logger.info("Starting fresh generation...")
        
    for idx in range(start_idx, total_q):
        row = df.iloc[idx]
        q_text = str(row[q_col]).strip()
        gt_text = str(row[a_col]).strip()
        
        paths = generate_k_paths(
            q_text, gen_pipe, tokenizer, 
        ...
        logger.info(f"Progress: {idx+1}/{total_q} | Q: {q_text[:40]}...")
