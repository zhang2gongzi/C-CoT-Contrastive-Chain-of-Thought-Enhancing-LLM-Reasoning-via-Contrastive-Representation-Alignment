...
# -------------------------- 1. 路径与参数配置 --------------------------
MODEL_PATH = "/home2/zzl/model_eval/modelscope_models/Qwen/Qwen-7B-Chat"  # Qwen模型路径
DATA_PATH = "/home2/zzl/C-CoT/database/commonsenseQA/train-00000-of-00001.parquet"  # 数据集路径
OUTPUT_DIR = "/home2/zzl/C-CoT/baseline/commonsenseQA/sc_cot/qwen_sc_results"  # 结果保存目录（与Llama区分）
SAMPLE_SIZE = 200  # 样本数量（与Llama测试保持一致）
...
def convert_choices_format(choices):
    """将NumPy数组格式转换为标准字典列表（与Llama版本完全一致）"""
    try:
...
def build_sc_prompt(question, choices):
    ...
    # Qwen提示词模板（更自然的指令表述）
    prompt = f"""请逐步推理并回答以下问题，最后务必以"Final Answer: X"的格式输出答案（X为A/B/C/D/E），不要添加其他内容。
...
def extract_answer(cot_response, choices):
    """从推理中提取答案（与Llama版本保持一致，确保评估标准统一）"""
...
def main():
    ...
    for sample_idx in range(len(df)):
        ...
        for gen_idx in range(NUM_GENERATIONS):
            ...
            # 每次生成使用不同的随机种子（与Llama保持相同的种子策略）
            generation_config = GenerationConfig(
                max_new_tokens=512,
                temperature=0.7,  # 相同的温度参数，确保随机性可比
                top_p=0.9,
                do_sample=True,
                pad_token_id=tokenizer.pad_token_id,
                eos_token_id=tokenizer.eos_token_id,
                seed=42 + gen_idx  # 相同的种子序列，增加结果可比性
            )
            ...
            # 移除提示词部分，只保留生成的推理
            if prompt in cot_response:
                cot_response = cot_response[len(prompt):].strip()
    ...
    # 保存完整结果
    save_path = f"{OUTPUT_DIR}/qwen_self_consistency_results.jsonl"
